...
export function DashboardShell({ user, children }: { user: CurrentUser; children: React.ReactNode }) {
  ...
  return (
    <div className="min-h-screen min-w-0 md:grid md:grid-cols-[248px_minmax(0,1fr)]">
      {mobileMenuOpen && <button aria-label="Close navigation menu" className="fixed inset-0 z-30 bg-[#10251c]/45 backdrop-blur-[2px] md:hidden" onClick={() => setMobileMenuOpen(false)} type="button" />}
      <aside aria-label="Main navigation" className={`fixed inset-y-0 left-0 z-40 flex w-[min(82vw,280px)] flex-col border-r border-[#e4e9e6] bg-white px-4 py-5 shadow-2xl transition-transform duration-200 ease-out md:w-[248px] md:translate-x-0 md:shadow-none ${mobileMenuOpen ? "translate-x-0" : "-translate-x-full"}`} id="mobile-navigation">
        ...
        <button aria-label="Close navigation menu" className="grid h-10 w-10 place-items-center rounded-xl text-[#66736d] hover:bg-[#f1f5f3] md:hidden" onClick={() => setMobileMenuOpen(false)} type="button"><X size={20} /></button>
        </div>
        ...
        <nav className="mt-9 min-h-0 flex-1 space-y-1 overflow-y-auto pb-4">
          ...
          {nav.map((item) => {
            ...
            return (
              <Link key={item.href} href={item.href} onClick={() => setMobileMenuOpen(false)} className={`flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-semibold transition ${active ? "bg-[#e9f4ef] text-[#0b6348]" : "text-[#66736d] hover:bg-[#f4f7f5] hover:text-[#26372f]"}`}>
      ...
      <div className="min-w-0 md:col-start-2">
        <header className="sticky top-0 z-20 flex h-16 items-center justify-between border-b border-[#e5eae7] bg-white/90 px-5 backdrop-blur md:px-8">
          <div className="flex items-center gap-2 md:hidden"><button aria-controls="mobile-navigation" aria-expanded={mobileMenuOpen} aria-label="Open navigation menu" className="grid h-10 w-10 place-items-center rounded-xl text-[#34453d] hover:bg-[#eef4f1]" onClick={() => setMobileMenuOpen(true)} type="button"><Menu size={22} /></button><span className="font-extrabold">FlashPOS</span></div>
