...
export function PosWorkspace({ products }: { products: PosProduct[] }) {
  ...
  const [paymentMethod, setPaymentMethod] = useState<"cash" | "gcash" | "credit_card">("cash");
    ...
    <div className="mt-7 grid items-start gap-5 xl:grid-cols-[minmax(0,1fr)_420px]">
      ...
      <form action={action} className="card overflow-hidden xl:sticky xl:top-24">
        ...
        <div className="border-b border-[#e5eae7] p-5">
          ...
          {channel === "walk_in" ? (
            <div className="mt-4 rounded-xl border border-[#dfe7e3] bg-[#f7faf8] p-3">
              <div className="flex items-center gap-2 text-xs font-extrabold text-[#315345]"><Store size={15} />Payment method</div>
              <div className="mt-3 grid grid-cols-3 gap-2">
                <button className={`rounded-lg border px-3 py-2 text-xs font-extrabold ${paymentMethod === "cash" ? "border-[#0f6b4f] bg-white text-[#0f6b4f] ring-1 ring-[#0f6b4f]" : "border-[#dce4df] bg-white text-[#748078]"}`} onClick={() => { setPaymentMethod("cash"); setPaymentReference(""); }} type="button">Cash</button>
                <button className={`rounded-lg border px-3 py-2 text-xs font-extrabold ${paymentMethod === "gcash" ? "border-[#1769e0] bg-blue-50 text-[#1769e0] ring-1 ring-[#1769e0]" : "border-[#dce4df] bg-white text-[#748078]"}`} onClick={() => { setPaymentMethod("gcash"); setPaymentReference(""); }} type="button">GCash</button>
                <button className={`rounded-lg border px-3 py-2 text-xs font-extrabold ${paymentMethod === "credit_card" ? "border-[#7c3aed] bg-violet-50 text-[#7c3aed] ring-1 ring-[#7c3aed]" : "border-[#dce4df] bg-white text-[#748078]"}`} onClick={() => { setPaymentMethod("credit_card"); setPaymentReference(""); }} type="button">Credit Card</button>
              </div>
              {paymentMethod === "cash" ? (
                <>
                  <label className="mt-3 block text-xs font-bold text-[#34453d]"><span className="mb-1.5 block">Cash received</span><span className="relative block"><span className="absolute left-3 top-1/2 -translate-y-1/2 text-sm font-bold text-[#7c8982]">₱</span><input className="field with-currency-prefix text-sm" name="amountTendered" value={amountTendered} onChange={(event) => setAmountTendered(event.target.value)} type="number" min="0" step="0.01" placeholder="0.00" required /></span></label>
                  {cashReceived > 0 && <div className="mt-3 flex justify-between border-t border-[#e0e8e3] pt-3 text-xs"><span className="font-semibold text-[#718078]">Change</span><span className="font-black text-[#0f6b4f]">{money(expectedChange)}</span></div>}
                </>
              ) : paymentMethod === "gcash" ? (
                <label className="mt-3 block text-xs font-bold text-[#34453d]"><span className="mb-1.5 block">GCash reference ID</span><input className="field text-sm" name="paymentReference" value={paymentReference} onChange={(event) => setPaymentReference(event.target.value)} placeholder="Enter payment reference" required /></label>
              ) : (
                <label className="mt-3 block text-xs font-bold text-[#34453d]"><span className="mb-1.5 block">Bank Name</span><input className="field text-sm" name="paymentReference" value={paymentReference} onChange={(event) => setPaymentReference(event.target.value)} placeholder="e.g. BDO, BPI, Metrobank" required /></label>
              )}
            </div>
          ) : (
        ...
        <div className="border-t border-[#e5eae7] bg-[#fbfcfb] p-5">
          ...
          <button className="btn-primary mt-4 w-full justify-center py-3.5 disabled:cursor-not-allowed disabled:opacity-50" disabled={pending || !cart.length || (channel === "walk_in" ? (paymentMethod === "cash" ? cashReceived < total : paymentReference.trim().length < (paymentMethod === "gcash" ? 4 : 2)) : !orderReference.trim())} type="submit">{pending ? "Completing order…" : channel === "walk_in" ? `Complete ${paymentMethod === "gcash" ? "GCash" : paymentMethod === "credit_card" ? "card" : "cash"} sale` : `Complete ${channels.find((item) => item.value === channel)?.label} order`}</button>
