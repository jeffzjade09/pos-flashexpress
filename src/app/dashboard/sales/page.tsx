...
type SaleItemRow = { id: string; product_name: string; unit_name: string; quantity: number; refunded_quantity: number; unit_price: number | string };
type SaleRow = {
  id: string;
...
export default async function SalesPage({ searchParams }: { searchParams: Promise<{ q?: string }> }) {
  ...
  const { data, error } = await supabase.from("sales").select("id, receipt_number, status, total_amount, refunded_amount, sales_channel, payment_method, external_order_id, payment_reference, fulfillment_status, completed_at, sale_items(id, product_name, unit_name, quantity, refunded_quantity, unit_price)").in("status", ["completed", "partially_refunded", "refunded", "voided"]).order("completed_at", { ascending: false }).limit(200);
