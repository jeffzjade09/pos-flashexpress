...
import { Eye, ReceiptText, Search, TrendingUp } from "lucide-react";
...
type SaleItemRow = { id: string; product_id: string; product_name: string; unit_name: string; quantity: number; refunded_quantity: number; conversion_to_piece: number; unit_price: number | string; cost_per_piece: number | string; discount_amount: number | string; refunded_discount_amount: number | string; tax_amount: number | string; refunded_tax_amount: number | string };
type SaleRow = {
  id: string;
...
export default async function SalesPage({ searchParams }: { searchParams: Promise<{ q?: string }> }) {
  ...
  // Profit is calculated from each line item's own cost/price snapshot (never the live
  // products table, which can change after the sale), excluding tax on both sides since
  // tax is collected on behalf of government, not company revenue. Reflects the same
  // 200-most-recent-transaction window (and search filter, if any) as the table below --
  // not an all-time or date-ranged figure. Restricted to super_admin, matching how
  // cost/margin data is already gated on the Reports page.
  type ProductProfit = { productName: string; netQuantity: number; netRevenue: number; netCost: number };
  const productProfits = new Map<string, ProductProfit>();
  if (user.role === "super_admin") {
    for (const sale of sales) {
      for (const item of sale.sale_items) {
        const netQuantity = Number(item.quantity) - Number(item.refunded_quantity);
        const netDiscount = Number(item.discount_amount) - Number(item.refunded_discount_amount);
        const current = productProfits.get(item.product_id) ?? { productName: item.product_name, netQuantity: 0, netRevenue: 0, netCost: 0 };
        current.netQuantity += netQuantity;
        current.netRevenue += Number(item.unit_price) * netQuantity - netDiscount;
        current.netCost += netQuantity * Number(item.conversion_to_piece) * Number(item.cost_per_piece);
        productProfits.set(item.product_id, current);
      }
    }
  }
  const profitByProduct = [...productProfits.values()].map((entry) => ({ ...entry, profit: entry.netRevenue - entry.netCost })).sort((a, b) => b.profit - a.profit);
  const totalProfit = profitByProduct.reduce((sum, entry) => sum + entry.profit, 0);

  return <div className="mx-auto max-w-[1400px]"><div className="flex flex-col justify-between gap-5 sm:flex-row sm:items-end"><div><p className="eyebrow">Sales ledger</p><h1 className="mt-2 text-3xl font-black tracking-[-0.03em]">Transactions</h1><p className="mt-2 text-sm text-[#718079]">Find receipts, reprint details, process returns, and track marketplace fulfillment.</p></div><Link href="/dashboard/pos" className="btn-primary">New sale</Link></div>

    {user.role === "super_admin" && (
      <div className="card mt-7 overflow-hidden">
        <div className="flex items-center justify-between border-b border-[#e7ece9] px-5 py-4">
          <div>
            <h2 className="font-extrabold">Profit by product</h2>
            <p className="mt-1 text-xs text-[#819087]">Net revenue minus cost of goods, across the {sales.length} transaction{sales.length === 1 ? "" : "s"} shown below{query ? " matching your search" : ""}. Excludes tax.</p>
          </div>
          <div className="text-right">
            <p className="text-[10px] font-bold uppercase tracking-wide text-[#87928c]">Total profit</p>
            <p className={`text-xl font-black ${totalProfit < 0 ? "text-red-600" : "text-[#0f6b4f]"}`}>{money(totalProfit)}</p>
          </div>
        </div>
        {profitByProduct.length === 0 ? (
          <p className="p-8 text-center text-sm text-[#87928c]">No product sales in view yet.</p>
        ) : (
            ...
            <table className="w-full min-w-[720px] text-left">
              <thead>
                <tr className="border-b border-[#e9eeeb] bg-[#fafcfa] text-[10px] font-extrabold uppercase tracking-[0.1em] text-[#87928c]">
                  ...
                  <th className="px-5 py-3 text-right">Cost of goods</th>
                  <th className="px-5 py-3 text-right">Profit</th>
                </tr>
              </thead>
              <tbody>
                {profitByProduct.map((entry, index) => (
                  <tr className="border-b border-[#edf0ee] last:border-0" key={index}>
                    <td className="px-5 py-3"><div className="flex items-center gap-2"><span className="grid h-8 w-8 shrink-0 place-items-center rounded-lg bg-[#edf5f1] text-[#0f6b4f]"><TrendingUp size={14} /></span><span className="text-xs font-extrabold">{entry.productName}</span></div></td>
                    <td className="px-5 py-3 text-right text-xs text-[#66736d]">{entry.netQuantity}</td>
                    <td className="px-5 py-3 text-right text-xs font-bold text-[#64736b]">{money(entry.netRevenue)}</td>
                    <td className="px-5 py-3 text-right text-xs text-[#8a958f]">{money(entry.netCost)}</td>
                    <td className={`px-5 py-3 text-right text-xs font-black ${entry.profit < 0 ? "text-red-600" : "text-[#0f6b4f]"}`}>{money(entry.profit)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
