...
export default async function ReportsPage({ searchParams }: { searchParams: Promise<ReportSearchParams> }) {
  ...
  const reportResults = validationError
    ? null
    : await Promise.all([
        supabase
          .from("sales")
  ...
  const saleData = reportResults?.[0].data ?? [];
  const salesError = reportResults?.[0].error ?? null;
  const itemData = reportResults?.[1].data ?? [];
  const expenseResult = reportResults?.[2] ?? { data: [] };
    ...
    <div className="page-print mx-auto max-w-[1400px]">
      ...
      <div className="flex flex-col justify-between gap-5 lg:flex-row lg:items-end">
        ...
        <div className="print-hidden flex flex-wrap gap-2">
          <Link className={`rounded-xl border border-[#dfe6e2] bg-white px-4 py-2.5 text-xs font-extrabold text-[#627068] hover:bg-[#f4f7f5] ${validationError ? "pointer-events-none opacity-50" : ""}`} href={range === "custom" ? `/api/export/sales?range=custom&start=${customStart}&end=${customEnd}` : "/api/export/sales"} aria-disabled={Boolean(validationError)}><span className="inline-flex items-center gap-1.5"><Download size={13} />Export</span></Link>
          ...
          <Link className={`rounded-xl border px-4 py-2.5 text-xs font-extrabold transition ${range === "custom" ? "border-[#0f6b4f] bg-[#0f6b4f] text-white" : "border-[#dfe6e2] bg-white text-[#627068] hover:bg-[#f4f7f5]"}`} href={`/dashboard/reports?range=custom&start=${customStart}&end=${customEnd}`}>Custom range</Link>
        </div>
      </div>

      {range === "custom" && (
        <form action="/dashboard/reports" className="print-hidden mt-4 flex flex-col gap-3 rounded-2xl border border-[#dfe6e2] bg-white p-4 sm:flex-row sm:items-end" method="get">
          <input name="range" type="hidden" value="custom" />
          <label className="flex-1 text-xs font-bold text-[#34453d]">
            <span className="mb-1.5 block">Start date</span>
            <input className="field text-sm" defaultValue={customStart} name="start" required type="date" />
          </label>
          <label className="flex-1 text-xs font-bold text-[#34453d]">
            <span className="mb-1.5 block">End date</span>
            <input className="field text-sm" defaultValue={customEnd} min={customStart} name="end" required type="date" />
          </label>
          <button className="btn-primary justify-center sm:min-w-28" type="submit">View report</button>
        </form>
      )}

      {validationError && <div className="mt-4 rounded-xl border border-red-200 bg-red-50 px-4 py-3 text-sm font-semibold text-red-700" role="alert">{validationError}</div>}
      ...
      {!validationError && !salesError && !sales.length && <div className="mt-6 rounded-xl border border-[#dfe6e2] bg-[#f7faf8] px-4 py-4 text-sm text-[#627068]" role="status"><p className="font-extrabold text-[#34453d]">No sales data for {rangeLabel.toLowerCase()}.</p><p className="mt-1 text-xs">Choose another date range to view completed transactions and report totals.</p></div>}
