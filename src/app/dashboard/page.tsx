...
export default async function DashboardPage() {
  ...
  const [{ count: productCount }, { data: stockData }, { data: saleData }, { data: itemData }] = await Promise.all([
    ...
    supabase.from("product_stock").select("id, name, variant, cost_per_piece, stock_on_hand, low_stock_threshold, is_active").eq("is_active", true).order("stock_on_hand"),
  ...
  const totalPiecesOnHand = (stockData ?? []).reduce((sum, item) => sum + Number(item.stock_on_hand), 0);
  const totalInventoryValue = (stockData ?? []).reduce(
    (sum, item) => sum + Number(item.stock_on_hand) * Number(item.cost_per_piece),
    0,
  );
        ...
        <Link className="card p-5 transition hover:-translate-y-0.5 hover:shadow-md" href="/dashboard/inventory">
          <div className="flex items-start justify-between gap-3">
            <div className="min-w-0">
              <p className="text-xs font-bold text-[#718078]">Inventory value</p>
              <p className="mt-3 truncate text-2xl font-black tracking-tight">{money(totalInventoryValue)}</p>
              <p className="mt-2 truncate text-xs text-[#8a958f]">{totalPiecesOnHand.toLocaleString()} pieces at cost</p>
            </div>
            <span className="grid h-10 w-10 shrink-0 place-items-center rounded-xl bg-teal-50 text-teal-700"><Boxes size={18} /></span>
          </div>
        </Link>
