...
export async function GET(_: Request, { params }: { params: Promise<{ kind: string }> }) {
  ...
  if (kind === "inventory") {
    ...
    const rows = (data ?? []).map((product) => [product.sku, product.barcode, product.name, product.variant_label, product.category_name, product.cost_per_piece, product.stock_on_hand, Number(product.cost_per_piece) * Number(product.stock_on_hand), product.low_stock_threshold, product.is_active]);
